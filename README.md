# Clinical EHR 30-Day Mortality & ICU Risk Stratification Engine
...
> **🏥 Machine Learning Triage & Clinical Risk Stratification Model for ICU Patient Prognosis with LightGBM & SHAP**

---

## 📌 Executive Summary & Mathematical Formulation

In Healthcare / Clinical Machine Learning, automated machine learning classification facilitates low-latency decision triage:
...
```
clinical-ehr-mortality-risk-predictor/
├── app.py                     # Streamlit interactive diagnostic dashboard
├── data/
│   ├── raw/
│   │   └── clinical_ehr_mortality_risk_predictor_dataset.csv   # Raw domain dataset
│   └── processed/
│       └── clinical_ehr_mortality_risk_predictor_processed.csv # Scaled and preprocessed matrix
├── models/
...
├── notebooks/
│   └── clinical_ehr_mortality_risk_predictor_pipeline.ipynb    # End-to-end Jupyter Notebook pipeline
├── reports/
...
```bash
git clone https://github.com/ArjunaFransesco/clinical-ehr-mortality-risk-predictor.git
cd clinical-ehr-mortality-risk-predictor
python -m venv venv
...
```bash
jupyter notebook notebooks/clinical_ehr_mortality_risk_predictor_pipeline.ipynb
```
